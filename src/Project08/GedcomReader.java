...
import java.io.BufferedReader; 
 ...
 import java.time.LocalDateTime;     //US38
import java.time.Period;
 ...
 public class GedcomReader { 
    ...
    //US30
     ...
     //Sprint 3
     //US38 
     public int getbirthdaysin30days(String date1) {   
         String month = date1.substring(5,7);  
         String day = date1.substring(8); 
         LocalDate today = LocalDate.now(); 
         LocalDate date2 = LocalDate.of(today.getYear(), Integer.parseInt(month), Integer.parseInt(day)); 
         long noofdays = ChronoUnit.DAYS.between(today, date2); 
         int days = (int) noofdays; 
 	     return days;  
 	}  
   ...
   //US15 
    ...
    //US39
     public int getAnniversariesin30Days(String date){
    	 String month = date.substring(5, 7);
    	 String day = date.substring(8);
    	 LocalDate today = LocalDate.now(); 
         LocalDate date1 = LocalDate.of(today.getYear(), Integer.parseInt(month), Integer.parseInt(day)); 
    	 long noofdays = ChronoUnit.DAYS.between(today, date1);
         int days = (int) noofdays; 
	     return days;  
     }
     ...
     public static void main(String[] args) { 
        ...
        if (recentdeathscount == 0) {                 
 		...
 		for (int i = 0; i < families.size(); i++) { 
            ...
            if ( gr.validateDate(marry) && husband.isAlive() && wife.isAlive() && family.getDivorced().equals("NA")) { 
                int daysFromAnniversary = gr.getAnniversariesin30Days(marry);  
                if ( daysFromAnniversary >= -30 && daysFromAnniversary <= 30 ) {  
                     System.out.println("\nLIST: US39: All anniversaries in the coming 30 days");  
                     System.out.println("+-----+--------------------+--------------------+-----------+");  
                     System.out.println("| ID  | HusbandName        |WifeName            | Marrydate |");  
                     System.out.println("+-----+--------------------+--------------------+-----------+");  
                     System.out.printf("|%-5s|%-20s|%-20s|%-11s|%n\n", family.getId(), husband.getName(), wife.getName(), family.getMarried() );  
                }
            }
